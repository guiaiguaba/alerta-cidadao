...
const CAT_EMOJI: Record<string, string> = {
  ...
  via:'🚧', vazam:'💧', lixo:'🗑️', ilum:'💡', buraco:'🕳️',
};
...
function Pill({ label, color, bg }: { label: string; color: string; bg: string }) {
  return (
    <span style={{ color, background: bg, border: `1px solid ${color}44`, borderRadius: 20, padding: '3px 9px', fontSize: 11, fontWeight: 700, whiteSpace: 'nowrap', display: 'inline-block' }}>
      {label}
    </span>
  );
}
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const s = localStorage.getItem('dash_theme');
  ...
  const { data: stats } = useQuery({
    ...
    enabled: !!dbUser, refetchInterval: 30000,
  });
  const { data: ocorrs, refetch } = useQuery({
    queryKey: ['ocorrs', filter],
    queryFn: () => api.get(`/ocorrencias?limit=30${filter ? `&status=${filter}` : ''}`).then(r => r.data),
    enabled: !!dbUser, refetchInterval: 15000,
  });
  ...
  const d = theme === 'dark';
  const c = {
    bg: d ? '#0F1117' : '#F4F5F7', sidebar: d ? '#13161F' : '#1A1D27',
    surface: d ? '#1A1D27' : '#fff', card: d ? '#21253A' : '#fff',
    border: d ? '#2A2E3F' : '#E5E7EB', text: d ? '#fff' : '#0F1117',
    muted: d ? '#8B90A0' : '#6B7280', hover: d ? '#ffffff08' : '#FF6B2B08',
  };
  ...
  if (loading || !dbUser) {
    return (
      <div style={{ minHeight: '100vh', background: '#0F1117', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ width: 36, height: 36, border: '3px solid #FF6B2B', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin .7s linear infinite' }} />
      </div>
    );
  }
      ...
      <aside style={{ width: 220, background: c.sidebar, display: 'flex', flexDirection: 'column', flexShrink: 0, borderRight: '1px solid rgba(255,255,255,.06)' }}>
        ...
        <div style={{ padding: '12px 14px', borderTop: '1px solid rgba(255,255,255,.06)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            ...
            <button onClick={() => signOut().then(() => router.replace('/login'))} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#8B90A0', fontSize: 15, padding: 4 }}>🚪</button>
          </div>
          <button onClick={() => { const n = theme === 'dark' ? 'light' : 'dark'; setTheme(n); localStorage.setItem('dash_theme', n); }}
            style={{ width: '100%', padding: '7px', borderRadius: 8, border: '1px solid rgba(255,255,255,.1)', background: 'rgba(255,255,255,.05)', cursor: 'pointer', color: '#8B90A0', fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        <header style={{ background: c.sidebar, borderBottom: `1px solid ${c.border}`, padding: '0 24px', height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ background: 'rgba(239,68,68,.15)', color: '#EF4444', border: '1px solid rgba(239,68,68,.3)', borderRadius: 20, fontSize: 10, fontWeight: 700, padding: '3px 8px' }}>● AO VIVO</span>
          </div>
          <span style={{ fontSize: 12, color: c.muted }}>Hoje, {now.getHours()}:{now.getMinutes().toString().padStart(2, '0')}</span>
        </header>
        ...
        <main style={{ flex: 1, overflow: 'auto', padding: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 20 }}>
            {[
              { val: s?.abertas ?? '—',      label: 'Críticas',       trend: '+2 última hora', color: '#EF4444' },
              ...
              { val: s?.resolvidas ?? '—',   label: 'Concluídas',     trend: '+8 vs mês ant.', color: '#22C55E' },
            ].map((st, i) => (
              <div key={i} style={{ background: c.card, borderRadius: 12, padding: '18px 20px', border: `1px solid ${c.border}`, borderTop: `3px solid ${st.color}` }}>
                <p style={{ fontSize: 34, fontWeight: 800, margin: '0 0 4px', lineHeight: 1, color: c.text }}>{st.val}</p>
                <p style={{ fontSize: 12, color: c.muted, margin: '0 0 6px' }}>{st.label}</p>
          ...
          <div style={{ background: c.card, borderRadius: 12, border: `1px solid ${c.border}` }}>
            <div style={{ padding: '16px 20px', borderBottom: `1px solid ${c.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontWeight: 700, fontSize: 15, color: c.text }}>Ocorrências Recentes</span>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {[null, 'aberta', 'em_andamento', 'resolvida'].map(f => (
                  <button key={f ?? 'all'} onClick={() => setFilter(f)} style={{
                    ...
                    border: `1px solid ${filter === f ? '#FF6B2B' : c.border}`,
                    background: filter === f ? 'rgba(255,107,43,.12)' : 'transparent',
                    color: filter === f ? '#FF6B2B' : c.muted,
            ...
            {items.length === 0 ? (
              ...
              return (
                <div key={o.id}
                  style={{ display: 'grid', gridTemplateColumns: '56px 1fr 160px 110px 110px 110px', padding: '13px 20px', borderBottom: `1px solid ${c.border}`, transition: 'background .12s' }}
                ...
                >
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 3, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <span style={{ fontWeight: 600, fontSize: 13, color: c.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{o.categoria_nome ?? 'Ocorrência'}</span>
                    </div>
                  ...
                  <div style={{ alignSelf: 'center' }}>
                    ...
                    {o.status === 'resolvida' && <span style={{ fontSize: 12, color: c.muted }}>Ver log</span>}
                  </div>
                </div>
              );
            })}
