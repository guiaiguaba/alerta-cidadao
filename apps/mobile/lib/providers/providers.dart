...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  Future<void> loginWithEmail({
    ...
    try {
      // Salvar slug antes da chamada (interceptor precisa)
      await _storage.saveTenantSlug(tenantSlug);
      ...
      final user = AppUser.fromJson({...me, 'tenantSlug': tenantSlug, 'tenantId': me['tenantId'] ?? ''});
      state = AuthState(user: user, isLoggedIn: true);
      await _registerFcmToken();
    } catch (e) {
  ...
  Future<void> _afterLoginSuccess(Map<String, dynamic> res, String tenantSlug) async {
    await _storage.saveTokens(
      accessToken:  res['accessToken'],
      refreshToken: res['refreshToken'],
    );
