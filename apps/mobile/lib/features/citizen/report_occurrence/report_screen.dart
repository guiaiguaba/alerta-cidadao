...
class _ReportScreenState extends ConsumerState<ReportScreen> {
  ...
  Future<void> _submit() async {
    ...
    // Verificação geográfica — bloquear se fora da área
    final permitido = await GeoBlocker.verificar(context, ref.read(apiClientProvider));
    if (!permitido) {
    ...
    try {
      if (isOnline) {
        ...
        // Upload foto se tivermos
        if (_photo != null) {
          try {
            await api.uploadMedia(occ['id'], _photo!, 'report');
          } catch (_) {
            // Falha no upload não impede a ocorrência
          }
        }
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Ocorrência registrada: ${occ['protocol']} ✅'),
