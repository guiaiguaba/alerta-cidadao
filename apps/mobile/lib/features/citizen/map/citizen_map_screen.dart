...
class _CitizenMapScreenState extends ConsumerState<CitizenMapScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      // Tentar sem filtro de status primeiro (mais compatível)
      final geoJson = await api.getMapData();
      ...
      setState(() {
        _loading = false;
        _error   = 'Não foi possível carregar as ocorrências. Verifique sua conexão.';
      });
      debugPrint('[Mapa] Erro ao carregar: $e');
    }
  }
      ...
      body: Stack(
        children: [
          ...
          if (_error != null && !_loading)
            Positioned.fill(
              child: Container(
                color: AppColors.base.withOpacity(0.92),
                child: Column(
